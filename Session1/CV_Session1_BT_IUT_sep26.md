...
این متن نسخه‌ی بازنویسی‌شده، تکمیل‌شده و آموزشی جلسه‌ی اول آشنایی با **بینایی ماشین (Computer Vision)** است. هدف من این نیست که صرفا مجموعه‌ای از تعریف‌ها، اسلایدها یا نکات پراکنده در اختیار شما قرار بدهم. تلاش کرده‌ام مسیر فکری جلسه را به شکلی بازسازی کنم که اگر کسی در کلاس حضور نداشته باشد نیز بتواند با خواندن این فایل، اجرای تمرین‌ها و بازی‌کردن با پارامترها، منطق موضوع را از پایه درک کند.

در این جلسه عمدا از یک مسیر مشخص حرکت می‌کنیم:

**انسان چگونه می‌بیند؟ → تصویر برای کامپیوتر چیست؟ → پردازش تصویر چیست؟ → لبه و ویژگی چیست؟ → روش‌های کلاسیک چرا محدود شدند؟ → ماشین چگونه یاد می‌گیرد؟ → شبکه عصبی دقیقا چه می‌کند؟ → CNN چگونه Feature می‌سازد؟ → YOLO چگونه وارد مسئله تشخیص شیء می‌شود؟**

این ترتیب مهم است. اگر مستقیم سراغ اجرای YOLO برویم، احتمالا می‌توانیم با چند خط کد یک Bounding Box روی تصویر بکشیم؛ اما هنوز نفهمیده‌ایم پشت آن چه اتفاقی افتاده است. هدف این درس‌نامه، عبور از «اجرای دستور» به سمت **فهم مهندسی سیستم** است.

## پیش‌نیاز و نصب

برای اجرای مثال‌ها به Python 3.9 یا بالاتر نیاز دارید. نصب کتابخانه‌های اصلی:

~~~bash
pip install opencv-python numpy ultralytics
~~~

اگر وب‌کم یا GPU در دسترس ندارید، بخش‌های نظری و مثال‌های مبتنی بر تصویر ثابت همچنان قابل اجرا هستند.

---

# فهرست مطالب

1. بینایی ماشین دقیقا چه مسئله‌ای را حل می‌کند؟
...
12. شبکه چگونه واقعا یاد می‌گیرد؟
13. CNN و استخراج سلسله‌مراتبی ویژگی
14. Overfitting و Underfitting
15. Transfer Learning، Fine-Tuning و Few/One/Zero-Shot
16. YOLO: از ایده تا اجرای عملی
17. ابزارهای کاری و آماده‌سازی محیط
18. تمرین‌های عملی OpenCV
19. اتصال وب‌کم و دوربین گوشی
20. Motion Detection
21. YOLO روی تصویر و وب‌کم
...
برای فهم بینایی ماشین، شروع از بینایی انسان بسیار کمک‌کننده است. آنچه ما «دیدن» می‌نامیم صرفا ورود نور به چشم نیست. نور از محیط بازتاب می‌شود، به شبکیه می‌رسد، به سیگنال عصبی تبدیل می‌شود و سپس مغز این سیگنال‌ها را در چندین مرحله پردازش می‌کند.
...
فرض کنید یک شیء دقیقا همرنگ پس‌زمینه باشد، نور و سایه‌ی مشابهی داشته باشد و بافت آن نیز با محیط ادغام شود. تشخیص مرز آن بسیار سخت‌تر می‌شود. منطق استتار نیز تا حد زیادی از همین موضوع استفاده می‌کند: کم‌کردن نشانه‌های بصری‌ای که مرز یک شیء را آشکار می‌کنند.
...
در قشر بینایی اولیه یا **V1**، گروه‌هایی از نورون‌ها نسبت به ویژگی‌هایی مثل جهت خطوط و لبه‌ها حساس‌اند. در CNNها نیز لایه‌های ابتدایی معمولا الگوهای ساده‌تری مانند لبه، جهت، کنتراست و Textureهای ابتدایی را بازنمایی می‌کنند و با عمیق‌تر شدن شبکه، بازنمایی‌ها پیچیده‌تر می‌شوند.
...
در **پردازش تصویر** تمرکز عمدتا روی خود داده‌ی تصویری است: روشن‌کردن تصویر، کاهش نویز، Grayscale، تغییر کنتراست، استخراج لبه، ساخت ماسک، Resize و Rotate.

در **بینایی ماشین** هدف اصلی استخراج اطلاعات و معنا از تصویر است؛ مثلا سیستم بگوید «در تصویر یک انسان وجود دارد»، «این شیء یک کلاه ایمنی است» یا «این جسم همان جسمی است که در فریم قبلی دیده شد».
...
### آیا هر سیستم CV حتما ابتدا Canny و Threshold اجرا می‌کند؟

خیر. در سیستم‌های کلاسیک، Pipeline اغلب به صورت صریح شامل preprocessing و feature extraction بود. در Deep Learning مدرن، شبکه می‌تواند بسیاری از featureها را مستقیما از داده یاد بگیرد. با این حال پردازش تصویر همچنان برای آماده‌سازی داده، اصلاح نور، Resize، Normalization، حذف نویز، ساخت Mask و کنترل کیفیت ورودی مهم است.
...
در تصویر 8-bit، مقدار 0 معمولا سیاه و 255 سفید است.
...
اگر رنگ برای مسئله اهمیت زیادی نداشته باشد، یک کانال به جای سه کانال می‌تواند محاسبات را ساده‌تر کند. تبدیل استاندارد رنگ به Gray صرفا «میانگین ساده سه کانال» نیست و معمولا وزن ادراکی متفاوتی برای کانال‌ها در نظر گرفته می‌شود.
...
اگر تعداد زیادی Pixel تغییر کند، احتمالا حرکتی رخ داده است.
...
> نکته: Regression معمولی غالبا یک مسئله Supervised است، چون Target عددی مشخص داریم.
...
نکته اصلی RL این نیست که «هیچ داده اولیه‌ای وجود ندارد»؛ تفاوت بنیادی این است که سیگنال یادگیری عمدتا از **تعامل و Reward** می‌آید.
...
وقتی می‌گوییم مدل «گربه را فهمیده»، نباید آن را دقیقا معادل فهم انسانی بگیریم. مدل یک **Representation عددی** می‌سازد که برای Classification یا Detection مفید است.
...
اگر کل Train Dataset یک بار در فرایند Training مصرف شود، معمولا می‌گوییم یک **Epoch** طی شده است.

داده‌ها معمولا به Batchهای کوچک‌تر تقسیم می‌شوند:
...
یک Kernel کوچک روی تصویر حرکت می‌کند. مثلا Kernel با ابعاد `3×3`. در هر موقعیت، محاسبه‌ای روی ناحیه محلی انجام می‌شود.
...
این توصیف آموزشی است و هر Channel لزوما مفهوم قابل نام‌گذاری انسانی ندارد.
...
> Overfitting صرفا به معنی «داده خیلی زیاد» نیست. داده‌ی **متنوع و باکیفیت بیشتر** معمولا به کاهش Overfitting کمک می‌کند.
...
به جای آموزش از صفر، از مدلی استفاده می‌کنیم که قبلا روی Dataset بزرگ آموزش دیده است.
...
ایده اصلی نسخه‌های اولیه این خانواده این بود که Detection را تا حد زیادی به صورت یک مسئله یکپارچه اجرا کنند و شبکه مستقیما از تصویر به Bounding Box و Class Prediction برسد.
...
برای هر Detection معمولا داده‌هایی از این جنس داریم:
...
اگر Bounding Box پیش‌بینی‌شده و Ground Truth تقریبا روی هم باشند، IoU بالا می‌شود.
...
`venv` وابستگی‌های Package را ایزوله می‌کند، اما معمولا **نسخه متفاوت Python را خودش نصب یا مدیریت نمی‌کند**. برای مدیریت چند Python Interpreter می‌توان از ابزارهایی مثل pyenv، conda یا uv استفاده کرد.
...
ترتیب Indexها تضمین‌شده نیست؛ ممکن است 0 وب‌کم داخلی، 1 دوربین مجازی و 2 دوربین موبایل باشد یا کاملا متفاوت باشد.
...
مثلا اگر در یک خط تولید:

- دوربین ثابت است؛
- نور ثابت است؛
- جسم همیشه Pose تقریبا یکسانی دارد؛
...
11. Dataset خوب، Label درست و Generalization از صرفا زیادکردن Epoch مهم‌ترند.
12. در مهندسی واقعی همیشه باید بپرسیم: **آیا اصلا برای این مسئله Deep Learning لازم است؟**
